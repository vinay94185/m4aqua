...
#define RMNET_DATA_NO_QOS_MODE 0
					...
					struct rmnet_logical_ep_conf_s *ep)
{
	if (!ep->egress_dev) {
		...
		rmnet_egress_handler(skb, ep, RMNET_DATA_NO_QOS_MODE);
	}
 ...
 * @qos_version: QoS version configured on this device
 *
 ...
 * Note: headroom expansion will incur a performance penalty.
 *
 * Return:
 *      - 0 on success
 *      - 1 on failure
				    ...
				    uint32_t qos_version)
{
	...
	if (qos_version & RMNET_IOCTL_QOS_MODE_6) {
			qos_header_length = sizeof(struct QMI_QOS_HDR_S);
	} else if (qos_version & RMNET_IOCTL_QOS_MODE_8) {
			qos_header_length = sizeof(struct qmi_qos_hdr8_s);
	}
	required_headroom = qos_header_length;
	...
	if (skb_headroom(skb) < required_headroom) {
        ...
        if (qos_version)
                rmnet_vnd_add_qos_header(skb, qos_version);
			  ...
			  uint32_t qos_version)
{
	...
	if (config->egress_data_format & RMNET_EGRESS_FORMAT_MAP) {
		switch (rmnet_map_egress_handler(skb, config, ep, qos_version)) {
